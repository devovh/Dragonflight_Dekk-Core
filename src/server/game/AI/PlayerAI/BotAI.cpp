...
void BotBGAI::ProcessHealth()
{
	...
	NearUnitVec const& needHealth = SearchNeedHealth(searchDist);
	if (needHealth.empty())
	{
...
void BotBGAI::ChaseTarget(Unit* pTarget, bool isMelee, float range)
{
	...
	if (isMelee)
	{
		if (me->IsStopped())
		{
			Position const& targetPos = pTarget->GetPosition();
			float rndOffset = frand(-float(M_PI_4) * 0.75f, float(M_PI_4) * 0.75f);
			Position pos;
			me->GetFirstCollisionPosition(me->GetDistance(targetPos) + range, me->GetRelativeAngle(&targetPos) + rndOffset);
...
NearObjectList BotBGAI::SearchGameObject(float range)
{
	...
	Position const& pos = me->GetPosition();
...
bool BotBGAI::NeedDireFlee()
{
	...
	if (me->GetHealthPct() < 20 && RangeEnemyListByTargetIsMe(BOTAI_SEARCH_RANGE * 0.6f).size() > 0)
		return true;
	NearPlayerVec const& rangeFriends = ExistFriendAttacker(BOTAI_SEARCH_RANGE * 0.6f);
	NearUnitVec const& rangeEnemys = RangeEnemyListByHasAura(0, BOTAI_SEARCH_RANGE * 0.6f);
...
bool BotBGAI::DoDireFlee()
{
	if (IsNotMovement())
		return false;
	NearUnitVec const& enemys = RangeEnemyListByHasAura(0, BOTAI_SEARCH_RANGE * 0.6f);
	if (enemys.empty())
		return false;
	NearPlayerVec const& friends = SearchFarFriend(BOTAI_SEARCH_RANGE * 0.8f, BOTAI_SEARCH_RANGE * 1.4f, true);
	if (friends.empty())
		return false;
...
bool BotBGAI::TargetIsMelee(Player* pTarget)
{
	...
	case CLASS_DRUID:
		return false;
	case CLASS_SHAMAN:
        return false;// (pTarget->FindTalentType() == 1);
	}
	return false;
}
...
bool BotBGAI::TargetIsHealth(Player* pTarget)
{
	...
	case CLASS_SHAMAN:
		//return (pTarget->FindTalentType() == 2);
	case CLASS_DRUID:
        return false;// (pTarget->FindTalentType() == 2);
	}
	return false;
}
