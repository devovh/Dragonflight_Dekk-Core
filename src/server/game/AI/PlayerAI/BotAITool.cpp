...
void BotUtility::AddArenaBotSpellsByPlayer(Player* player)
{
	...
	{
		if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(ARENA_PLAYER_BOT_AURA, DIFFICULTY_NONE))
...
Position BotUtility::GetPositionFromGroup(Player* pCenterPlayer, ObjectGuid self, Group* pGroup)
{
	if (!pCenterPlayer || !pGroup || !pCenterPlayer->IsInWorld())
		return Position();
	Position const& centerPos = pCenterPlayer->GetPosition();
	uint32 index = 0;
	uint32 count = 0;
	...
	float angle = pCenterPlayer->GetOrientation() + onceAngle * float(index);
        float distX = centerPos.GetPositionX() + distance * cosf(angle);
        float distY = centerPos.GetPositionY() + distance * sinf(angle);
...
void BotUtility::ProcessGroupRingMovement(Player* pCenterPlayer, BOTAI_WORKTYPE aiType)
{
	...
	float meleeDistance = pCenterUnit->GetObjectScale() + 5.0f;
	for (uint32 i = 0; i < meleePlayers.size(); i++)
	{
		...
		float distX = centerPos.GetPositionX() + meleeDistance * cosf(angle);
		float distY = centerPos.GetPositionY() + meleeDistance * sinf(angle);
		float distZ = centerPos.GetPositionZ();
	...
	float rangeDistance = pCenterUnit->GetObjectScale() + BOTAI_FLEE_JUDGE + NEEDFLEE_CHECKRANGE;
	for (uint32 i = 0; i < rangePlayers.size(); i++)
	{
		...
		float angle = pCenterUnit->GetOrientation() + onceAngle * float(i);
		float distX = centerPos.GetPositionX() + rangeDistance * cosf(angle);
		float distY = centerPos.GetPositionY() + rangeDistance * sinf(angle);
		float distZ = centerPos.GetPositionZ();
		distZ = pCenterUnit->GetMap()->GetHeight(pCenterUnit->GetPhaseShift(), distX, distY, distZ);
		Position resultPos(distX, distY, distZ, pCenterUnit->GetOrientation());
		Position pos;
...
{
	if (range < 3.0f || range <= pRefUnit->GetObjectScale() || pRefUnit->GetDistance(pTargetUnit->GetPosition()) >= range)
		return false;
	...
	for (float angle = 0; angle < (float(M_PI) * 2.0f); angle += float(M_PI_4))
	{
		...
		float distX = pRefUnit->GetPositionX() + calcRange * cosf(angle);
		float distY = pRefUnit->GetPositionY() + calcRange * sinf(angle);
		...
		while (!pTargetUnit->IsWithinLOS(distX, distY, distZ))
		{
			if (calcRange <= once)
				break;
			calcRange -= once;
			distX = pRefUnit->GetPositionX() + calcRange * cosf(angle);
			distY = pRefUnit->GetPositionY() + calcRange * sinf(angle);
...
void BotAIStoped::UpdatePosition(uint32 diff)
{
	...
	{
		SyncPosition(currentPos, CMSG_MOVE_START_FORWARD);
	...
	else
	{
		...
		{
			SyncPosition(currentPos, CMSG_MOVE_STOP);
			m_updateTick = 0;
			m_lastPosition = currentPos;
			me->StopMoving();
		}
...
Position BotAIHorrorState::FindNewHorrorPos(BotBGAIMovement* movement)
{
	...
	for (float angle = 0.0f; angle < (float(M_PI) * 2.0f); angle += onceAngle)
	{
		...
		{
			float distX = me->GetPositionX() + (moveMaxDistance*dist) * cosf(angle);
			float distY = me->GetPositionY() + (moveMaxDistance*dist) * sinf(angle);
...
void BotAIFlee::AddCruxFlee(uint32 durTime, Unit* pRefUnit, BotBGAIMovement* pMovement)
{
	if (!pMovement || !pRefUnit || !durTime)
		return;
	float fleeDistance = pRefUnit->GetObjectScale() + BOTAI_RANGESPELL_DISTANCE;
	float onceAngle = float(M_PI) * 2.0f / 8.0f;
	Position targetPos;
	float selectAngle = me->GetOrientation() * (-1);
	float maxDist = 0.0f;
...
bool BotAIFlee::CanFleeToTargetPlayer(Player* player)
{
	...
	case CLASS_DEATH_KNIGHT:
		return false;
	case CLASS_PALADIN:
        return false;// player->FindTalentType() == 0;
	case CLASS_MAGE:
	case CLASS_WARLOCK:
	case CLASS_HUNTER:
	case CLASS_PRIEST:
	...
	case CLASS_SHAMAN:
	case CLASS_DRUID:
		return false;//player->FindTalentType() != 1;
	}
	return false;
}
...
bool BotAICheckDuel::CheckDuel()
{
	...
	//Player const *player = ObjectAccessor::FindPlayer(duel->Initiator);
	//if (!player || player->IsPlayerBot())
	//{
