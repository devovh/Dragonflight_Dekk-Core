...
void BotDruidAI::ProcessHealthSpell(Unit* pTarget)
{
	...
	if (ProcessActive())
		return;
	NearUnitVec const& friends = SearchLifePctByFriendRange(pTarget, 70);
	if (friends.size() > 1)
	{
...
void BotDruidAI::ProcessBalanceCombat(Unit* pTarget)
{
	...
	NearUnitVec const& rangeEnemy = RangeEnemyListByHasAura(0, BOTAI_RANGESPELL_DISTANCE);
	...
	NearUnitVec const& targetRangeEnemy = RangeEnemyListByTargetRange(pTarget, NEEDFLEE_CHECKRANGE);
...
bool BotDruidAI::TryStartControlCommand()
{
	...
	SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(DruidGuard_Control, DIFFICULTY_NONE);
...
{
	NearUnitVec const& enemys = RangeEnemyListByTargetIsMe(BOTAI_SEARCH_RANGE);
