...
void BotHunterAI::ResetBotAI()
{
	...
	if (HunterShot_QMLShot == 0 && m_BotTalentType == 1 && me->GetLevel() == 60)
	{
...
void BotHunterAI::ProcessRangeSpell(Unit* pTarget)
{
	...
	NearUnitVec const& selMeEnemys = RangeEnemyListByTargetIsMe(BOTAI_RANGESPELL_DISTANCE);
...
bool BotHunterAI::TryStartControlCommand()
{
	...
	SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(HunterTrap_FarFrozen, DIFFICULTY_NONE);
...
bool BotHunterAI::CastRangeSpell(Unit* pTarget)
{
	...
	if (me->GetDistance(pTarget->GetPosition()) <= 9)
		return false;
	NearUnitVec const& targetRanges = RangeEnemyListByTargetRange(pTarget, NEEDFLEE_CHECKRANGE);
	...
	if (pTarget->GetGUID() == me->GetGUID())
	{
		if (m_BotTalentType == 1 && HunterShot_Aim && TryCastSpell(HunterShot_Aim, pTarget) == SpellCastResult::SPELL_CAST_OK)
			return true;
		if (m_BotTalentType == 1 && HunterShot_QMLShot && me->GetLevel() == 60 && TryCastSpell(HunterShot_QMLShot, pTarget) == SpellCastResult::SPELL_CAST_OK)
			return true;
