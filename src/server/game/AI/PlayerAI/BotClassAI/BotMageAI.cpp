...
{
	m_BotTalentType = PlayerBotSetting::FindPlayerTalentType(me);
}
...
void BotMageAI::ResetBotAI()
{
	...
	if (MageConfine_Freeze == 0 && m_BotTalentType == 2 && me->GetLevel() == 60)
	{
...
bool BotMageAI::NeedFlee()
{
	if (m_Flee.Fleeing())
		return true;
	NearUnitVec const& nearEnemys = RangeEnemyListByTargetIsMe(NEEDFLEE_CHECKRANGE);
	if (me->InArena())
	{
...
void BotMageAI::ProcessFlee()
{
	...
	NearUnitVec const& enemys = RangeEnemyListByTargetIsMe(NEEDFLEE_CHECKRANGE);
	if (enemys.size() > 0)
	{
	...
	if (enemys.size() > 0 && TryCastSpell(MageAssist_Stealth, me) == SpellCastResult::SPELL_CAST_OK)
	{
		...
		NearUnitVec const& cancelTargets = RangeEnemyListByTargetIsMe(BOTAI_RANGESPELL_DISTANCE);
	...
	NearUnitVec const& stealTargets = RangeEnemyListByHasAura(0, BOTAI_RANGESPELL_DISTANCE);
...
{
	NearUnitVec const& enemys = RangeEnemyListByTargetIsMe(BOTAI_RANGESPELL_DISTANCE);
	float healthPct = me->GetHealthPct();
...
{
	NearUnitVec const& enemys = RangeEnemyListByTargetIsMe(8.0f);
	float healthPct = me->GetHealthPct();
	if (healthPct <= 50 && enemys.size() > 0 && GetManaPowerPer() > 50 && !me->HasAura(MageGuard_MagicShield) && TryCastSpell(MageGuard_MagicShield, me) == SpellCastResult::SPELL_CAST_OK)
		return true;
	if (Unit* pTarget = me->GetSelectedUnit())
	{
		...
		if (me->InArena())
		{
			NearUnitVec const& nearEnemys = RangeEnemyListByHasAura(0, BOTAI_RANGESPELL_DISTANCE);
			for (Unit* pUnit : nearEnemys)
			{
...
bool BotMageAI::ProcessPolymorph(Unit* pTarget)
{
	if (!pTarget || m_NeedReserveCtrlSpell)
		return false;
	NearUnitVec const& enemys = RangeEnemyListByHasAura(0, BOTAI_SEARCH_RANGE);
...
bool BotMageAI::ProcessFreezeSpell(Unit* pTarget)
{
	...
	{
		if (m_BotTalentType == 2 && MageConfine_Freeze && me->GetLevel() == 60 &&
...
{
	Position const& telePos = GetTeleportSpellPos();
...
void BotMageAI::OnCastPolymorph(Unit* pTarget)
{
	...
	me->AttackStop();
	NearUnitVec const& friends = SearchFriendTargetIsTarget(pTarget);
...
bool BotMageAI::TryStartControlCommand()
{
	...
	NearUnitVec const& friends = SearchFriend();
	...
	SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(MageConfine_ToSheep , DIFFICULTY_NONE);
...
Position BotMageAI::GetTeleportSpellPos()
{
	...
	{
		Position const& pos = me->GetFirstCollisionPosition(BOTAI_RANGESPELL_DISTANCE, angle);
...
uint32 BotMageAI::TryCastSummonRiteSpell()
{
	...
	SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(MageIDLE_SummonRite, DIFFICULTY_NONE);
	if (!spellInfo || spellInfo->IsPassive())
