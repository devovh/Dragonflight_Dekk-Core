...
bool BotPaladinAI::ProcessControl(Unit* pTarget)
{
	...
	{
		NearUnitVec const& enemys = RangeEnemyListByHasAura(0, BOTAI_TOTEMRANGE);
		if (enemys.size() <= 1)
			return false;
		for (Unit* pUnit : enemys)
		{
...
void BotPaladinAI::ProcessMeleeSpell(Unit* pTarget)
{
	...
	if (PaladinAssist_UpPower && meLife > 80 && TryCastSpell(PaladinAssist_UpPower, me) == SpellCastResult::SPELL_CAST_OK)
		return;
	NearUnitVec const& tryKills = RangeEnemyListByHasAura(0, BOTAI_RANGESPELL_DISTANCE);
...
bool BotPaladinAI::TryStartControlCommand()
{
	...
	SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(PaladinAssist_Confession, DIFFICULTY_NONE);
