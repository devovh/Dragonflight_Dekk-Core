...
void BotPriestAI::ResetBotAI()
{
	...
	InitializeSpells();
	if (PriestGuard_GuardSoul == 0 && m_BotTalentType == 1 && me->GetLevel() == 60)
	{
		me->LearnSpell(47788, false);
		PriestGuard_GuardSoul = FindMaxRankSpellByExist(47788);
	}
	if (PriestGuard_RecoverMana == 0 && m_BotTalentType == 2 && me->GetLevel() == 60)
	{
...
bool BotPriestAI::ProcessNormalSpell()
{
	...
	if (me->HasAura(m_UseMountID))
		return false;
	NearUnitVec const& needHealthPlayers = SearchNeedHealth(BOTAI_RANGESPELL_DISTANCE);
...
void BotPriestAI::ProcessFlee()
{
	...
	NearUnitVec const& enemys = RangeEnemyListByHasAura(0, NEEDFLEE_CHECKRANGE);
	if (enemys.size() > 1)
	{
		if (TryCastSpell(PriestGuard_AOEFear, me) == SpellCastResult::SPELL_CAST_OK)
			return;
	}
	NearUnitVec const& rangeEnemys = RangeEnemyListByTargetIsMe(BOTAI_RANGESPELL_DISTANCE);
	...
	else if (m_BotTalentType == 1)
	{
		...
		{
			NearUnitVec const& needHeals = SearchLifePctByFriendRange(me, 75);
			if (needHeals.size() > 1)
			{
		...
		if (healthPct < 60 && rangeEnemys.size() > 0 && TryCastSpell(PriestGuard_SelfHealth, me) == SpellCastResult::SPELL_CAST_OK)
			return;
		if (healthPct < 8 && rangeEnemys.size() > 0 && me->GetLevel() == 60 && TryCastSpell(PriestGuard_GuardSoul, me) == SpellCastResult::SPELL_CAST_OK)
			return;
	}
...
bool BotPriestAI::ProcessArenaHealthMember()
{
	...
	NearUnitVec const& friends = SearchFriend(BOTAI_RANGESPELL_DISTANCE);
...
bool BotPriestAI::ProcessManaBurn()
{
	...
	NearUnitVec const& enemys = RangeEnemyListByHasAura(0, BOTAI_RANGESPELL_DISTANCE);
...
{
	if (m_BotTalentType != 2 || me->GetLevel() != 60 || !PriestGuard_RecoverMana)
		return false;
	uint32 manaPct = GetManaPowerPer();
