...
bool BotRogueAI::ProcessMeleeBlind(Unit* pTarget)
{
	...
	for (Unit* player : enemys)
	{
		if (player == pTarget || !CanBlind(player))
			continue;
		NearUnitVec const& friends = SearchFriendTargetIsTarget(player);
		if (!friends.empty())
			continue;
...
void BotRogueAI::ProcessMeleeSpell(Unit* pTarget)
{
	...
	{
		NearUnitVec const& nearEnemys = RangeEnemyListByHasAura(0, NEEDFLEE_CHECKRANGE);
...
bool BotRogueAI::TryStartControlCommand()
{
	...
	SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(RogueAssist_Blind, DIFFICULTY_NONE);
...
void BotRogueAI::OnCastFlash(Unit* pTarget)
{
	...
	Position const& pos = pTarget->GetPosition();
