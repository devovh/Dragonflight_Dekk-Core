...
bool BotShamanAI::ProcessUpWeapon()
{
	...
	SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(weaponSpellEntry, DIFFICULTY_NONE);
...
bool BotShamanAI::TryStartControlCommand()
{
	...
	SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(ShamanAssist_Frog, DIFFICULTY_NONE);
	if (!spellInfo || spellInfo->IsPassive())
	{
		m_CruxControlTarget = ObjectGuid::Empty;
		return false;
...
{
	bool hasEarth = false;
	NearUnitVec const& nearEnemys = RangeEnemyListByTargetIsMe(BOTAI_RANGESPELL_DISTANCE);
	if (nearEnemys.empty())
	{
