...
bool BotWarlockAI::TryStartControlCommand()
{
	...
	SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(WarlockAssist_BaseFear, DIFFICULTY_NONE);
...
bool BotWarlockAI::ProcessFear(Unit* pTarget)
{
	...
	if (me->InArena())
	{
		NearUnitVec const& targetMeEnemys = RangeEnemyListByHasAura(0, NEEDFLEE_CHECKRANGE);
...
void BotWarlockAI::ProcessRangeSpell(Unit* pTarget)
{
	...
	NearUnitVec const& enemys = RangeEnemyListByTargetRange(pTarget, NEEDFLEE_CHECKRANGE);
	if (enemys.size() > 1)
	{
...
uint32 BotWarlockAI::TryCastSummonRiteSpell()
{
	...
	SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(WarlockIDLE_SummonRite, DIFFICULTY_NONE);
	if (!spellInfo || spellInfo->IsPassive())
