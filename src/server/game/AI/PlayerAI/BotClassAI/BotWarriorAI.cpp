...
{
	NearPlayerVec const& farFriends = SearchFarFriend(12, BOTAI_RANGESPELL_DISTANCE, false);
	if (!farFriends.empty())
	{
	...
	NearUnitVec const& range8Players = RangeEnemyListByTargetIsMe(8.0f);
...
void BotWarriorAI::ProcessRageMeleeSpell(Unit* pTarget)
{
	...
	if (!pTarget->HasAura(WarriorWeaponRage_Backfillet) && TryCastSpell(WarriorWeaponRage_Backfillet, pTarget) == SpellCastResult::SPELL_CAST_OK)
		return;
	NearUnitVec const& playerVec8Range = RangeEnemyListByHasAura(0, NEEDFLEE_CHECKRANGE);
...
void BotWarriorAI::ProcessDefanceMeleeSpell(Unit* pTarget)
{
	...
	if (TryCastSpell(WarriorDefance_Fuchou, pTarget) == SpellCastResult::SPELL_CAST_OK)
		return;
	NearUnitVec const& range8Players = RangeEnemyListByTargetIsMe(NEEDFLEE_CHECKRANGE);
	...
	if (ragePer >= 40 && TryCastSpell(WarriorDefance_HPojia, pTarget) == SpellCastResult::SPELL_CAST_OK)
		return;
	NearUnitVec const& range30Players = RangeEnemyListByTargetIsMe(BOTAI_RANGESPELL_DISTANCE);
...
bool BotWarriorAI::ProcessFullAttack(Unit* pTarget)
{
	if (!pTarget)
		return false;
	NearUnitVec const& targetMeEnemys = RangeEnemyListByTargetIsMe(BOTAI_RANGESPELL_DISTANCE);
