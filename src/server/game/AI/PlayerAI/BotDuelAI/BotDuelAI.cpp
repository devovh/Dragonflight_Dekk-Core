...
{
	if (!me->duel || me->duel->State == DuelState::DUEL_STATE_COMPLETED || !me->duel->Opponent || !me->duel->Initiator)
	{
		m_Movement->ClearMovement();
		me->SetSelection(ObjectGuid::Empty);
		me->AttackStop();
...
{
	if (!me->duel || me->duel->State == DuelState::DUEL_STATE_COMPLETED || !me->duel->Opponent)
		return false;
	if (me->duel->State == DuelState::DUEL_STATE_IN_PROGRESS)
		return false;
...
bool BotDuelAI::TargetIsMelee(Player* pTarget)
{
	...
	{
	case CLASS_PALADIN:
		return (PlayerBotSetting::FindPlayerTalentType(me) != 0);
	case CLASS_WARRIOR:
	case CLASS_ROGUE:
	case CLASS_DEATH_KNIGHT:
		return true;
	...
	case CLASS_DRUID:
		return false;
	case CLASS_SHAMAN:
		return (PlayerBotSetting::FindPlayerTalentType(me) == 1);
	}
	return false;
}
...
bool BotDuelAI::TargetIsHealth(Player* pTarget)
{
	...
	case CLASS_PALADIN:
		return (PlayerBotSetting::FindPlayerTalentType(me) == 0);
	case CLASS_PRIEST:
		return (PlayerBotSetting::FindPlayerTalentType(me) != 2);
	case CLASS_SHAMAN:
		return (PlayerBotSetting::FindPlayerTalentType(me) == 2);
	case CLASS_DRUID:
		return (PlayerBotSetting::FindPlayerTalentType(me) == 2);
