...
bool DuelMageAI::ProcessFreezeSpell(Unit* pTarget)
{
	...
	{
		if (m_BotTalentType == 2 && MageConfine_Freeze && me->GetLevel() == 60 && !pTarget->HasAura(MageConfine_Freeze) && TryCastSpell(MageConfine_Freeze, pTarget) == SpellCastResult::SPELL_CAST_OK)
			return true;
