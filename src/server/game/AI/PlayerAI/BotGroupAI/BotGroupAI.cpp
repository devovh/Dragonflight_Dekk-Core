    ...
    for (uint8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; ++i)
    {
        if (Bag* pBag = me->GetBagByPos(i))
        {
            for (uint32 j = 0; j < pBag->GetBagSize(); j++)
            {
                ...
                std::string itemLink = BotUtility::BuildItemLinkText(pTemplate);
                std::string outputText = itemLink + " x " + std::to_string(pItem->GetCount());
                needSendText.push_back(outputText);
            }
        }
    }

 for (const std::string& text : needSendText)
    {
        me->Whisper(text, Language::LANG_COMMON, srcPlayer);
    }
}
...
void BotGroupAI::ProcessUpequip(Player* srcPlayer, std::string& equipLink)
{
  ...
  //  if (pItem == nullptr)
    ...
    // If the item is ammo, set the ammo slot.
  ...
  //  if (pItem->GetTemplate()->GetInventoryType() == InventoryType::INVTYPE_AMMO)
   ...
   // else
    ...
    //        consoleToUtf8(std::string("Eß∞‹◊∞±∏"), outString);
  ...
  //  std::string outString;
  //  consoleToUtf8(std::string("l…a¶◊∞±∏"), outString);
 //   me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());
//}
...
void BotGroupAI::ProcessUnequip(Player* srcPlayer, std::string& equipLink)
{
	...
	for (uint8 slot = EquipmentSlots::EQUIPMENT_SLOT_HEAD; slot < EquipmentSlots::EQUIPMENT_SLOT_END; slot++)
	{
		...
		{
			std::string outString;
			consoleToUtf8(std::string("C?D¬Eß∞‹"), outString);
			me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());
			return;
		}
		...
		std::string outString;
		consoleToUtf8(std::string("l…a¶C?D¬"), outString);
		me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());
		return;
	}
	std::string outString;
...
void BotGroupAI::ProcessDestroyItem(Player* srcPlayer, std::string& equipLink)
{
	...
	std::string outString;
	consoleToUtf8(std::string("l…a¶∂SµÙ"), outString);
	me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());*/
}
...
void BotGroupAI::ProcessTradeItem(Player* srcPlayer, std::string& equipLink)
{
	...
	//{
	//	std::string outString;
	//	consoleToUtf8(std::string("ŒNAª”DO‚∏ˆµRlﬂ"), outString);
	//	me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());
	//	return;
	//}
	...
	//{
	//	std::string outString;
	//	consoleToUtf8(std::string("Aª”DzSEL?ªN◊"), outString);
	//	me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());
	//	return;
	//}
	...
	//{
	//	std::string outString;
	//	consoleToUtf8(std::string("O‚∏ˆµRlﬂNNl≠∑L…DCA¡À"), outString);
	//	me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());
	//	return;
	//}
	...
	////{
	////	std::string outString;
	////	consoleToUtf8(std::string("ŒT∑®∑L…DO‚∏ˆµRlﬂ"), outString);
	...
	//{
	//	std::string outString;
	//	consoleToUtf8(std::string("∑L…DCA¡À"), outString);
	//	me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());
	//}
	//else
	//{
	//	std::string outString;
	//	consoleToUtf8(std::string("Aª∑®‘U∑L∂´Œ˜¡À"), outString);
	//	me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());
	//}
}
...
void BotGroupAI::ProcessUseItem(Player* srcPlayer, std::string& equipLink)
{
	...
	{
		std::string outString;
		consoleToUtf8(std::string("Eß∞‹Ea”A"), outString);
		me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());
		return;
	}
	std::string outString;
	consoleToUtf8(std::string("l…a¶Ea”A"), outString);
	me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());*/
}
...
void BotGroupAI::ProcessTalent(Player* srcPlayer, std::string& talentText)
{
	...
	std::string outString;
	consoleToUtf8(std::string("«DªªEe∏lÕel…"), outString);
	//me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());
...
void BotGroupAI::ProcessSummonRiteSpell(Player* srcPlayer)
{
	...
	{
		std::string outString;
		consoleToUtf8(std::string("ƒz«∞ŒT∑®zSELOUª?N«E?L?"), outString);
		//me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());
		return;
	}
	...
	if (castSpellID)
	{
		...
		std::string outString;
		consoleToUtf8(std::string("OUª?N«E?CÙ∂ZL?"), outString);
		//me->Whisper(outString, Language::LANG_COMMON, srcPlayer->GetGUID());
	}
	else
	{
...
void BotGroupAI::UpdateAI(uint32 diff)
{
	...
	if (TryTeleportToMaster())
		return;
	if (m_MasterPlayer && m_MasterPlayer->duel && !(m_MasterPlayer->duel->State == DuelState::DUEL_STATE_COMPLETED))
	{
		if (!me->duel)
		{
...
bool BotGroupAI::IsNotSelect(Unit* pTarget)
{
	if (!pTarget || !pTarget->IsAlive())
		return true;
	if (pTarget->HasAura(27827)) // (27827 lCEe÷Æªe …nƒ¡ÀRÕˆsÛ)
		return true;
...
void BotGroupAI::ProcessFollowToMaster()
{
	...
	if (m_MasterPlayer->duel && !(m_MasterPlayer->duel->State == DuelState::DUEL_STATE_COMPLETED))
	{
		m_Movement->ClearMovement();
		return;
	}
...
SpellCastResult BotGroupAI::PetTryCastSpell(uint32 spellID, Unit* pTarget, bool force)
{
	...
	if (!spellInfo || spellInfo->IsPassive())
		return SpellCastResult::SPELL_FAILED_UNKNOWN;
	Spell* spell = new Spell(pPet, spellInfo, TriggerCastFlags{});
...
bool BotGroupAI::TargetIsStealth(Player* pTarget)
{
	if (!pTarget)
		return false;
	// (1784 µ¡‘Ù«±DD || 5215 µ¬¬lN¡«±DD || 66 ∑®E¶NtDŒ || 58984 ∞µNaNt∂›)
	if (pTarget->HasAura(1784) || pTarget->HasAura(5215) ||
		pTarget->HasAura(66) || pTarget->HasAura(58984))
	{
		if (!me->CanSeeOrDetect(pTarget, false, true)) // Oe?‚«±DD
			return true;
	}
	return false;
}
