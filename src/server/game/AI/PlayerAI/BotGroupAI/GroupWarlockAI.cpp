...
void GroupWarlockAI::ProcessFlee()
{
	...
	if (enemys.size() <= 2 && !TargetIsFear(pRndPlayer))
	{
		if (enemys.size() > 1)
		{
			if (WarlockAssist_AOEFear && TargetIsNotDiminishingByType2(pRndPlayer, DiminishingGroup::DIMINISHING_DISORIENT))
			{
				me->StopMoving();
		...
		{
			if (!BotUtility::ControllSpellDiminishing || TargetIsNotDiminishingByType2(pRndPlayer, DiminishingGroup::DIMINISHING_DISORIENT))
			{
				me->StopMoving();
	...
	if (WarlockPetSuccubus_Charm)
	{
		...
		for (Unit* pEnemy : enemys)
		{
			...
			if (SearchFriendTargetIsTarget(pEnemy, BOTAI_RANGESPELL_DISTANCE).size() > 0)
				continue;
			//if (TargetIsNotDiminishingByType2(pEnemy, DiminishingGroup::DIMINISHING_DISORIENT))
			//{
