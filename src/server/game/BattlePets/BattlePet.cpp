...
void BattlePet::ChangeQuality(Player* OwningPlayer, uint32 NewQuality, bool bSendUpdate)
{
    Quality = NewQuality;
    UpdateStats();

    if (OwningPlayer && bSendUpdate)
    {
        OwningPlayer->GetSession()->SendBattlePetUpdates(this);
    }
}

void BattlePet::ChangeLevel(Player* OwningPlayer, uint32 NewLevel, bool bSendUpdate)
{
    Level = std::clamp<uint32>(NewLevel, 0, BATTLE_PET_MAX_LEVEL);
...
std::shared_ptr<BattlePetInstance> BattlePetInstance::CloneForBattle(std::shared_ptr<BattlePetInstance> const& battlePet)
{
    ...
    battlePetInstance->OriginalCreature = battlePet->OriginalCreature;
    for (uint8 i = 0; i < MAX_PET_BATTLE_ABILITIES; ++i)
