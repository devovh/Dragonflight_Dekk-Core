...
void PetBattle::Finish(uint32 winnerTeamID, bool aborted, bool ignoreAbandonPenalty)
{
    ...
    {
        Player* player = ObjectAccessor::GetObjectInWorld(Teams[currentTeamID]->PlayerGuid, static_cast<Player*>(nullptr));
        if (!player)
            continue;
        ...
        for (size_t currentPetID = 0; currentPetID < Teams[currentTeamID]->TeamPetCount; ++currentPetID)
        {
            ...
            if (winnerTeamID == currentTeamID && availablePetCount && BattleType == PET_BATTLE_TYPE_PVE && currentPet->IsAlive() && currentPet->Level < BATTLE_PET_MAX_LEVEL && FightedPets.find(currentPet->ID) != FightedPets.end())
            {
                ...
                if (currentPet->XP + xpEarn >= xpToNextLevel)
                {
                    ...
                    if (const BattlePetSpeciesEntry* speciesInfo = sBattlePetSpeciesStore.LookupEntry(currentPet->Species))
                    {
                        player->UpdateCriteria(CriteriaType::BattlePetReachLevel, currentPet->Level, speciesInfo->PetTypeEnum, currentPet->Species);
                        player->UpdateCriteria(CriteriaType::PlayerObtainPetThroughBattle, speciesInfo->ID, currentPet->Level, currentPet->Species);
                    }

                    AchievementEntry const* slot2_achievement = sAchievementStore.AssertEntry(7433);
                    AchievementEntry const* slot3_achievement = sAchievementStore.AssertEntry(6566);
                    switch (currentPet->Level)
                    {
                    case 3:
                        /// Quest progress for 12 x Level Up!
                        player->KilledMonsterCredit(65876);
                        if (currentPet != nullptr || currentPet->Slot < 2)
                            player->CompletedAchievement(slot2_achievement);
                        break;
                    case 5:
                        if (currentPet != nullptr || currentPet->Slot < MAX_PET_BATTLE_SLOTS)
                            player->CompletedAchievement(slot3_achievement);
                        break;
                    }
                }
                else
            ...
            if (winnerTeamID == currentTeamID)
            {
                ...
                if (speciesInfo)
                {
                    player->UpdateQuestObjectiveProgress(QUEST_OBJECTIVE_DEFEATBATTLEPET, speciesInfo->ID, 1, InitialWildPetGUID);
                }
            }
            ...
            if (BattleType == PET_BATTLE_TYPE_PVE && currentTeamID == winnerTeamID && Teams[currentTeamID]->CapturedPet != PET_BATTLE_NULL_ID)
            {
                ...
                if (const BattlePetSpeciesEntry* speciesInfo = sBattlePetSpeciesStore.LookupEntry(Pets[Teams[currentTeamID]->CapturedPet]->Species))
                {
            ...
            {
                if (Creature* trainer = Hoff::FindMapCreature(player->GetMap(), Teams[PET_BATTLE_PVE_TEAM_ID]->OwnerGuid))
                {
                    player->UpdateQuestObjectiveProgress(QUEST_OBJECTIVE_WINPETBATTLEAGAINSTNPC, trainer->GetEntry(), 1, trainer->GetGUID());
                }
            }
        }
    }
        ...
        if (Teams[currentTeamID])
        {
            ...
            if (!player)
            {
                if (BattleType == PET_BATTLE_TYPE_PVE && PveBattleType == PVE_PET_BATTLE_WILD)
                {
                    ...
                    {
                        std::shared_ptr<BattlePetInstance> currentPet = Teams[currentTeamID]->TeamPets[i];
                        if (!currentPet)
                            continue;

                        Player* player = ObjectAccessor::GetObjectInWorld(Teams[PET_BATTLE_TEAM_1]->PlayerGuid, static_cast<Player*>(nullptr));
                        if (!player)
                            continue;

                        Creature* wildPet = Hoff::FindMapCreature(player->GetMap(), currentPet->OriginalCreature);
                        if (!wildPet)
                            continue;
