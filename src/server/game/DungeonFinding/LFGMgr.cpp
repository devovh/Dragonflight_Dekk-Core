...
#include "Hoff.h" // < Hoff
#include "ConditionMgr.h" // < Hoff

namespace lfg
{

    LFGDungeonData::LFGDungeonData() : id(0), name(), map(0), type(0), expansion(0), group(0), contentTuningId(0),
        difficulty(DIFFICULTY_NONE), RequiredPlayerConditionId(0), seasonal(false), x(0.0f), y(0.0f), z(0.0f), o(0.0f), requiredItemLevel(0), requiredChromieTime(0)
    {
    }
...
// < Fluxurion
    ...
    */
    ...
    lfg::LfgDungeonSet LFGMgr::GetRandomAndSeasonalDungeons(Player const* player, uint8 expansion, uint32 contentTuningReplacementConditionMask)
    {
        uint8 level = player->GetLevel();
