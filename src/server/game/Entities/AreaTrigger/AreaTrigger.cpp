...
bool AreaTrigger::Create(uint32 areaTriggerCreatePropertiesId, Unit* caster, Unit* target, SpellInfo const* spell, Position const& pos, int32 duration, SpellCastVisual spellVisual, ObjectGuid const& castId, AuraEffect const* aurEff)
{
    ...
    SetUpdateFieldValue(areaTriggerData.ModifyValue(&UF::AreaTriggerData::SpellID), spell ? spell->Id : 0);
    SetUpdateFieldValue(areaTriggerData.ModifyValue(&UF::AreaTriggerData::SpellForVisuals), spell ? spell->Id : 0);
...
float AreaTrigger::GetSphereRadius() const
{
    if (!_shape.IsSphere() && !_shape.IsCylinder())
        return .0f;

    return _shape.SphereDatas.Radius;
}

void AreaTrigger::SetSphereRadius(float radius, float radiusTarget /*= .0f*/)
{
    if (!_shape.IsSphere() && !_shape.IsCylinder())
        return;

    _shape.SphereDatas.Radius = radius;
    _shape.SphereDatas.RadiusTarget = radiusTarget ? radiusTarget : radius;

    _maxSearchRadius = _shape.GetMaxSearchRadius();
    SetUpdateFieldValue(m_values.ModifyValue(&AreaTrigger::m_areaTriggerData).ModifyValue(&UF::AreaTriggerData::BoundsRadius2D), GetMaxSearchRadius());
}
