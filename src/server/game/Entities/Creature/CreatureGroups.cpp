...
void FormationMgr::RemoveCreatureFromGroup(CreatureGroup* group, Creature* member)
{
    ...
    if (group->IsEmpty())
    {
        ...
        ASSERT(itr != map->CreatureGroupHolder.end(), "Not registered group {} in map {}", group->GetLeaderSpawnId(), map->GetId());
...
void FormationMgr::LoadCreatureFormations()
{
    ...
    for (ObjectGuid::LowType leaderSpawnId : leaderSpawnIds)
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "creature_formation contains leader spawn {} which is not included on its formation, removing", leaderSpawnId);
