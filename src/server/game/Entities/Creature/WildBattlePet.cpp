...
void WildBattlePetMgr::ReplaceCreature(Creature* creature, WildPetPoolTemplate* wTemplate, WildBattlePetPool* pTemplate)
{
    ...
    // Set creature flags
    replacementCreature->SetNpcFlag(UNIT_NPC_FLAG_WILD_BATTLE_PET);
    replacementCreature->SetUnitFlag(UNIT_FLAG_IMMUNE_TO_NPC);
...
void WildBattlePetMgr::EnableWildBattle(Creature* creature)
{
    ...
    battlePetInstance->Flags = 0;
    battlePetInstance->Health = 20000;
    battlePetInstance->OriginalCreature = creature->GetGUID();
...
void WildBattlePetMgr::Depopulate(WildBattlePetPool* pTemplate)
{
    ...
    Map* CreatureMap = nullptr;
    Creature* validCreature = nullptr;

    for (Creature* creature : pTemplate->ToBeReplaced)
    {
        if (creature && creature->IsCreature())
        {
            CreatureMap = creature->GetMap();
            if (CreatureMap)
            {
                validCreature = creature;
                break;
            }
        }
    }

    if (!validCreature || !CreatureMap)
    {
        return;
    }
    ...
    {
        Creature* unit = Hoff::FindMapCreature(CreatureMap, creature.first);
        if (unit)
        {
...
void WildBattlePetMgr::UnreplaceCreature(Creature* creature)
{
    ...
    const ObjectGuid creatureGUID = creature->GetGUID();

    WildBattlePetPool* pTemplate = creatureMap->GetWildBattlePetPool(creature);
    if (!pTemplate || pTemplate->ReplacedRelation.find(creatureGUID) == pTemplate->ReplacedRelation.end())
        return;

    const ObjectGuid replacementGUID = pTemplate->ReplacedRelation[creatureGUID];
    Creature* replacementCreature = Hoff::FindMapCreature(creatureMap, replacementGUID);
    ...
    pTemplate->ReplacedRelation.erase(creatureGUID);
}
...
void WildBattlePetMgr::EnterInBattle(Creature* creature)
{
    ...
    //creature->SetTarget(_player->GetGUID());
    creature->SendClearTarget();
    creature->SetUnitFlag(UNIT_FLAG_PACIFIED | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_UNINTERACTIBLE); // Immuned to all
...
{
    creature->RemoveUnitFlag(UNIT_FLAG_PACIFIED | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_UNINTERACTIBLE);
    creature->SetControlled(false, UNIT_STATE_ROOT);
    creature->_petBattleId.Clear();
    creature->SetDisplayId(creature->GetNativeDisplayId());

    if (IsWildPet(creature) && p_Defeated)
    {
        creature->KillSelf();
    }
}
