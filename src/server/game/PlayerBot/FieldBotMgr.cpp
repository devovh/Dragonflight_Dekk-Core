...
void FieldWarfare::OverFieldActing()
{
	...
	for (SessionMap::const_iterator itSession = allSession.begin(); itSession != allSession.end(); itSession++)
	{
		...
		Position homePosition = Position(player->m_homebind);
		if (BotFieldAI* pFieldAI = dynamic_cast<BotFieldAI*>(player->GetAI()))
		{
			pFieldAI->SetDrivingPVP(false);
			pFieldAI->SetWarfareTarget(NULL);
			//pFieldAI->SetTeleport(homePosition);
...
{
    auto sessionList = sWorld->GetAllSessions();
    for (auto itSession = sessionList.begin(); itSession != sessionList.end(); ++itSession)
    {
...
void FieldWarfare::TeleportToTargetVisible(Player* pBot, Player* pTarget)
{
	...
	int8 maxLevel = (pTarget->GetLevel() >= 55) ? 60 : pTarget->GetLevel() + 5;
    ...
    if (pBot->GetLevel() < minLevel || pBot->GetLevel() > maxLevel)
	{
		...
		schedule2.parameter3 = PlayerBotSetting::FindPlayerTalentType(0) + 1;
	...
	do
	{
		...
		distX = pTarget->GetPositionX() + (BOTAI_FIELDTELEPORT_DISTANCE * 1.7f) * cosf(targetAngle);
		distY = pTarget->GetPositionY() + (BOTAI_FIELDTELEPORT_DISTANCE * 1.7f) * sinf(targetAngle);
		distZ = pTarget->GetPositionZ();
...
Position FieldWarfare::GetEvadePosition(Player* centerPlayer, Player* evadePlayer)
{
	...
	{
		float distX = evadePlayer->GetPositionX() + (BOTAI_FIELDTELEPORT_DISTANCE * 4) * cosf(angle);
		float distY = evadePlayer->GetPositionY() + (BOTAI_FIELDTELEPORT_DISTANCE * 4) * sinf(angle);
...
void FieldBotMgr::TeleportToTargetVisible(BotFieldAI* pFieldAI, Player* pTarget)
{
	...
	else
	{
		...
		do
		{
			...
			float targetAngle = frand(0, float(M_PI) * 2.0f);
                        distX = pTarget->GetPositionX() + (BOTAI_FIELDTELEPORT_DISTANCE * 1.2f) * cosf(targetAngle);
                        distY = pTarget->GetPositionY() + (BOTAI_FIELDTELEPORT_DISTANCE * 1.2f) * sinf(targetAngle);
                        distZ = pTarget->GetPositionZ();
			distZ = pTarget->GetMap()->GetHeight(pTarget->GetPhaseShift(), distX, distY, distZ);
		} while (!MapManager::IsValidMapCoord(pTarget->GetMapId(), distX, distY, distZ, pTarget->GetOrientation()));
	}
...
void FieldBotMgr::UpdateTeleport()
{
	...
	{
		Position homePosition = Position(leavePlayer->m_homebind);
if (leavePlayer)
