...
void PlayerBotMgr::OnPlayerBotLogin(WorldSession* pSession, Player* pPlayer)
{
    ...
    if (pSession)
    {
        ...
        consoleToUtf8(std::string(" ÉD Dß"), outString);
...
void PlayerBotMgr::OnPlayerBotLogout(WorldSession* pSession)
{
    ...
    std::string outString;
    consoleToUtf8(std::string("»úC÷CËDÂDß"), outString);
...
void PlayerBotMgr::LoginFriendBotByPlayer(Player* pPlayer)
{
   ...
   /* 	std::vector<ObjectGuid> friends = pPlayer->GetSocial()->Get();
    	...
    	consoleToUtf8(std::string("|cffff8800ElNé°cÎT·¨OU»?sAÓN»úC÷CËÉDDß?L|r"), allonlineText);
...
void PlayerBotMgr::AddNewPlayerBot(bool faction, Classes prof, uint32 count)
{
    ...
    {
        std::string allonlineText;
        consoleToUtf8(std::string("|cffff8800ËuÓD»úC÷CËOËsLNNl­C«?zÔÚDßL¬ÎT·¨ÉDDßDÂ»úC÷CË?L|r"), allonlineText);
        sWorld->SendGlobalText(allonlineText.c_str(), NULL);
    }
}
...
void PlayerBotMgr::AddNewAccountBot(bool faction, Classes prof)
{
    ...
    std::string allonlineText;
#ifdef INCOMPLETE_BOT
    consoleToUtf8(std::string("|cffff8800ElNé°cÎT·¨OU»?ÉDDß×Ô?¨OËsL?ÇÉ«|r"), allonlineText);
    sWorld->SendGlobalText(allonlineText.c_str(), NULL);
    return;
#endif
    if (prof == CLASS_NONE)
    ...
    consoleToUtf8(std::string("|cffff8800A»ÓDONµ?sÍÄaDrÍ¬OóÓSµÄÖ¸¶¨Ö°NµµÄ×Ô?¨OËsL?ÇÉ«|r"), allonlineText);
    sWorld->SendGlobalText(allonlineText.c_str(), NULL);
}
...
void PlayerBotMgr::AddNewPlayerBotToBG(TeamId team, uint32 minLV, uint32 maxLV, BattlegroundTypeId bgTypeID)
{
    ...
    std::string allonlineText;
    consoleToUtf8(std::string("|cffff8800ËuÓD»úC÷CËOËsLNNl­C«?zÔÚDßL¬ÎT·¨LÓCëDÂ»úC÷CËµ?O?l?ÖD?L|r"), allonlineText);
    sWorld->SendGlobalText(allonlineText.c_str(), NULL);
}
...
////
////	std::string allonlineText;
////	consoleToUtf8(std::string("|cffff8800ËuÓD»úC÷CËOËsLNNl­C«?zÔÚDßL¬ÎT·¨LÓCëDÂ»úC÷CËµ?µRDÂlÇ¶ÓÁDÖD?L|r"), allonlineText);
////	sWorld->SendGlobalText(allonlineText.c_str(), NULL);
////}
...
void PlayerBotMgr::AddNewPlayerBotToAA(TeamId team, BattlegroundTypeId bgTypeID, uint32 bracketID, uint32 aaType)
{
    ...
    std::string allonlineText;
    consoleToUtf8(std::string("|cffff8800ËuÓD»úC÷CËOËsLNNl­C«?zÔÚDßL¬ÎT·¨LÓCëDÂ»úC÷CËµ?lsLLl?ÖD?L|r"), allonlineText);
    sWorld->SendGlobalText(allonlineText.c_str(), NULL);
}
