...
PlayerBotSession::PlayerBotSession(uint32 id, std::string& name, AccountTypes sec, uint8 expansion, time_t mute_time, LocaleConstant locale, uint32 recruiter, bool isARecruiter) :
    ...
    m_NoWorldTick(0),
    m_AccountBot(false),
    WorldSession(id, std::string(name), 0, nullptr, SEC_PLAYER, 8, 0, "Wn64", LOCALE_zhCN, 0, false) // AT_AUTH_FLAG_NONE, 0)
{
    SetAddress("playbot");
}
...
{
	bool updateResult = PlayerBotSession::Update(diff, map);
	ProcessNoWorld(diff);
	m_LastCastTime -= diff;
	if (m_LastCastTime <= 0)
	{
