...
void WorldStateMgr::LoadFromDB()
{
    ...
    do
    {
        ...
        for (std::string_view mapIdToken : Trinity::Tokenize(mapIds, ',', false))
        {
            ...
            if (!mapId)
            {
                TC_LOG_DEBUG("sql.sql", "Table `world_state` contains a world state {} with non-integer MapID ({}), map ignored",
                    id, mapIdToken);
                continue;
            }
            ...
            {
                TC_LOG_DEBUG("sql.sql", "Table `world_state` contains a world state {} with invalid MapID ({}), map ignored",
                    id, *mapId);
                continue;
            }
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Table `world_state` contains a world state {} with nonempty MapIDs ({}) but no valid map id was found, ignored",
                id, mapIds);
            continue;
        }
        ...
        if (!worldState.MapIds.empty())
        {
            for (std::string_view areaIdToken : Trinity::Tokenize(areaIds, ',', false))
            {
                ...
                if (!areaId)
                {
                    TC_LOG_DEBUG("sql.sql", "Table `world_state` contains a world state {} with non-integer AreaID ({}), area ignored",
                        id, areaIdToken);
                    continue;
                }
                ...
                if (!areaTableEntry)
                {
                    TC_LOG_DEBUG("sql.sql", "Table `world_state` contains a world state {} with invalid AreaID ({}), area ignored",
                        id, *areaId);
                    continue;
                }
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "Table `world_state` contains a world state {} with AreaID ({}) not on any of required maps, area ignored",
                        id, *areaId);
                    continue;
                }
            ...
            {
                TC_LOG_DEBUG("sql.sql", "Table `world_state` contains a world state {} with nonempty AreaIDs ({}) but no valid area id was found, ignored",
                    id, areaIds);
                continue;
            }
        }
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Table `world_state` contains a world state {} with nonempty AreaIDs ({}) but is a realm wide world state, area requirement ignored",
                id, areaIds);
        }
    ...
    if (result)
    {
        do
        {
            ...
            if (!worldState)
            {
                TC_LOG_DEBUG("sql.sql", "Table `world_state_value` contains a value for unknown world state {}, ignored", worldStateId);
                continue;
            }
