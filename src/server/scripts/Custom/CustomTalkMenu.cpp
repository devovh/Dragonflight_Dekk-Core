...
bool CustomTalkMenu::ProcessSummon(CustomItem* pMenuItem, Player* player)
{
    ...
    if (player->SummonCreature(entry, player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), player->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 300s))
		return true;
	return false;
}
...
bool CustomTalkMenu::ProcessAura(CustomItem* pMenuItem, Player* player)
{
	...
	if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId, Difficulty::DIFFICULTY_NONE))
	{
        AuraCreateInfo AuraInfo = AuraCreateInfo(player->GetGUID(), spellInfo, Difficulty::DIFFICULTY_NONE, spellInfo->GetSchoolMask(), player);
		...
		if (allGroup && pGroup)
		{
			...
			for (Group::MemberSlot const& slot : memList)
			{
				...
				if (!pGroupPlayer || !pGroupPlayer->IsAlive() || player->GetMap() != pGroupPlayer->GetMap())
					continue;
				Aura::TryRefreshStackOrCreate(AuraInfo, MAX_EFFECT_MASK);
			}
		}
		else
			Aura::TryRefreshStackOrCreate(AuraInfo, MAX_EFFECT_MASK);
