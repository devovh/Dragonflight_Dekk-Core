    ...
    struct npc_chromie_startAI : public NullCreatureAI
    {
        ...
        bool OnGossipHello(Player* player) override
        {
            ...
            if (InstanceScript* instance = me->GetInstanceScript())
            {
                ...
                if (player->IsGameMaster()) // GM instance state override menu
...
public:
    ...
    struct npc_chromie_middleAI : public NullCreatureAI
    {
        ...
        void MoveInLineOfSight(Unit* unit) override
        {
            ...
            {
                bool bPlagueCratesFinished = Instance->GetData(DATA_INSTANCE_PROGRESS) == CRATES_DONE;
                bool bPlayerFinishedCrateQuest = player->GetQuestStatus(QUEST_DISPELLING_ILLUSIONS) == QUEST_STATUS_COMPLETE;
                bool bPlayerIsNearby = me->GetDistance2d(player) < 40.0f;
                if (bPlagueCratesFinished && bPlayerIsNearby)
                {
