    ...
    public:
        ...
        struct boss_echo_of_tyrandeAI : public BossAI
        {
            ...
            void JustEngagedWith(Unit* who) override
            {
                ...
                events.ScheduleEvent(EVENT_MOONBOLT, 1s);
                events.ScheduleEvent(EVENT_STARDUST, 15s);
                events.ScheduleEvent(EVENT_MOONLANCE, 5s);
            }
...

            {
                ...
                {
                    events.Reset();
                    events.ScheduleEvent(EVENT_LUNAR_GUIDANCE, 10s);
                    events.ScheduleEvent(EVENT_EYES_OF_THE_GODDESS, 5s);
                    PhaseCount++;
                }
                ...
                {
                    events.Reset();
                    events.ScheduleEvent(EVENT_TEARS_OF_ELUNE, 1s);
                    PhaseCount++;
                }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    case EVENT_MOONBOLT:
                        ...
                        events.ScheduleEvent(EVENT_MOONBOLT, 2s);
                        break;
                    case EVENT_STARDUST:
                    {
                        events.ScheduleEvent(EVENT_STARDUST, 9s);
                        break;
                    }
                    case EVENT_MOONLANCE:
                        if (SelectTarget(SelectTargetMethod::MaxDistance, 0, 100.0f, true))
                        {
                    ...
                    case EVENT_LUNAR_GUIDANCE:
                        ...
                        events.ScheduleEvent(EVENT_MOONBOLT, 2s);
                        events.ScheduleEvent(EVENT_STARDUST, 10s);
                        events.ScheduleEvent(EVENT_MOONLANCE, 11s);
                        break;
                    case EVENT_TEARS_OF_ELUNE:
                        DoCastAOE(SPELL_TEARS_OF_ELUNE);
                        events.ScheduleEvent(EVENT_TEARS_OF_ELUNE_AOE, 5s);
                        events.ScheduleEvent(EVENT_MOONBOLT, 2s);
                        events.ScheduleEvent(EVENT_STARDUST, 9s);
                        events.ScheduleEvent(EVENT_MOONLANCE, 11s);
                        break;
                    ...
                    case EVENT_EYES_OF_THE_GODDESS:
                        ...
                        events.ScheduleEvent(EVENT_EYES_OF_THE_GODDESS, 22s);
                        break;
                    }
                }
            }
    ...
    public:
        ...
        struct npc_moonlance_singleAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                {
                    if (Unit* target = tyrande->AI()->SelectTarget(SelectTargetMethod::MaxDistance, 0, 100.0f, true))
                    {
        ...
        CreatureAI* GetAI(Creature* creature) const override
        {
            return GetEndTimeAI<npc_moonlance_singleAI>(creature);
        }
};
    ...
    public:
        ...
        struct npc_eye_of_the_goddessAI : public ScriptedAI
        {
            ...
            void FillCirclePath(Position const& centerPos, float radius, float z, Movement::PointsArray& path, bool clockwise)
            {
                ...
                float angle = centerPos.GetAbsoluteAngle(me->GetPositionX(), me->GetPositionY());
        ...
        CreatureAI* GetAI(Creature* creature) const override
        {
            return GetEndTimeAI<npc_eye_of_the_goddessAI>(creature);
        }
};
