        ...
        struct boss_murozondAI : public BossAI
        {
            ...
            void InitializeAI() override
            {
                ...
                if (Rewardchest != nullptr)
                    PhasingHandler::AddPhase(Rewardchest, 50, true);
            }
            ...
            void JustEngagedWith(Unit* who) override
            {
                ...
                events.ScheduleEvent(EVENT_INFINITE_BREATH, 22s);
                events.ScheduleEvent(EVENT_TEMPORAL_BLAST, 12s);
                events.ScheduleEvent(EVENT_DISTORTION_BOMB, 5s);
                ...
                if (Map* map = me->GetMap())
                {
                    ...
                    for (Map::PlayerList::const_iterator itr = Players.begin(); itr != Players.end(); ++itr)
                    {
                        if (Player* player = itr->GetSource())
                        {
                            ...
                            Creature* clone = player->SummonCreature(46956, player->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN, 0s, 0);
            ...
            {// this will give the achievement to players in heroic difficulty
                ...
                for (Map::PlayerList::const_iterator itr = PlayerList.begin(); itr != PlayerList.end(); ++itr)
                {
                    if (Player* player = itr->GetSource())
                    {
                        player->CompletedAchievement(sAchievementStore.LookupEntry(6117));
                    }
                }
            }
            ...
            void DoAction(int32 action) override
            {
                ...
                case ACTION_MUROZOND_START:
                {
                    ...
                    me->m_Events.AddEvent(new MurozondLandEvent(*me, MurozondLandPos), me->m_Events.CalculateTime(2s));
                    me->m_Events.AddEvent(new MurozondStartAttack(*me), me->m_Events.CalculateTime(5s));
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    case EVENT_DISTORTION_BOMB:
                        if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 100, true))
                        {
                            ...
                            events.ScheduleEvent(EVENT_DISTORTION_BOMB,  1s);
                        }
                        break;
...
class npc_murozond_trash : public CreatureScript
{
    public:
        ...
        CreatureAI* GetAI(Creature* creature) const override
        {
            return GetEndTimeAI<npc_murozond_trashAI>(creature);
        }
};

struct  go_hourglass_of_time : public GameObjectAI
{
    go_hourglass_of_time(GameObject* go) : GameObjectAI(go) { }
    ...
    bool OnGossipHello(Player* pPlayer) override
    {
        ...
        {
            if (Creature* murozond = me->FindNearestCreature(NPC_MUROZOND_ENTRY, 200.0f))
...
void AddSC_boss_murozond()
{
    ...
    RegisterGameObjectAI(go_hourglass_of_time);
}
